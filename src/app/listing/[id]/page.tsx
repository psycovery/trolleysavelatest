...
import { formatPounds, formatBestBefore, calcSaving } from '@/lib/utils'
...
export default function ListingDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (data) supabase.from('listings').update({ view_count: (data.view_count ?? 0) + 1 }).eq('id', data.id)
    }
    load()
  }, [id])
  ...
  const saving = calcSaving(listing.asking_price ?? 0, 0)
    ...
    <>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 pb-24">
        ...
        <div className="grid md:grid-cols-2 gap-8 items-start">
          {/* Left — image + seller */}
          <div>
            <div className={`relative h-72 rounded-[14px] overflow-hidden mb-4 flex items-center justify-center border border-gray-100
              ${listing.is_donation ? 'bg-[#E8F8F0]' : 'bg-gray-50'}`}>
              {listing.image_url ? (
          ...
          <div>
            ...
            {/* Supermarket comparison placeholder */}
            {!listing.is_donation && (
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              {[
                ...
                ['Delivery', listing.delivery_method === 'both' ? 'Post or collect'
                  : listing.delivery_method === 'post' ? 'Royal Mail only' : 'Collect only'],
              ].map(([label, value]) => (
